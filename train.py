    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in [-1, 0]:
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                #     torch.save(ckpt, w / 'best_{:03d}.pt'.format(epoch))
                # if best_fitness == fi:
                ...
                #     torch.save(ckpt, w / 'best_f.pt')
                # if epoch == 0:
                #     torch.save(ckpt, w / 'epoch_{:03d}.pt'.format(epoch))
                # if ((epoch+1) % 25) == 0:
                #     torch.save(ckpt, w / 'epoch_{:03d}.pt'.format(epoch))
                # if epoch >= (epochs-5):
                #     torch.save(ckpt, w / 'last_{:03d}.pt'.format(epoch))
                # elif epoch >= 420:
                #     torch.save(ckpt, w / 'last_{:03d}.pt'.format(epoch))
                del ckpt
...
def parse_opt(known=False):
    ...
    parser.add_argument('--project', default=ROOT / 'repetition/', help='save to project/name')
    parser.add_argument('--name', default='AIR', help='save to project/name')
