...
class AvgAttentionModule(nn.Module):
    def __init__(self, c1, reduction=16):
        super(AvgAttentionModule, self).__init__()
...
class MaxAttentionModule(nn.Module):
    def __init__(self, c1, reduction=16):
        super(MaxAttentionModule, self).__init__()
        mid_channel = c1 // reduction
    ...
    def forward(self, x):
        ...
        x_w = self.max_pool_y(x).permute(0, 1, 3, 2)
...
class CBAM_bak(nn.Module):
    def __init__(self, c1,c2):
        super(CBAM_bak, self).__init__()
...
class CBAM(nn.Module):
    def __init__(self, c1,c2):
        ...
        self.avg_attention = AvgAttentionModule(c1)
        self.max_attention = MaxAttentionModule(c1)
        self.conv2d = nn.Conv2d(in_channels=c1 * 2, out_channels=c1, kernel_size=7, stride=1, padding=3)
        self.act=nn.SiLU()

    def forward(self, x):
        avgout = self.avg_attention(x) 
        maxout = self.max_attention(x)
        out = torch.cat([avgout, maxout], dim=1)
        out = self.act(self.conv2d(out))
        return out
...
class PSAModule(nn.Module):
    ...
    def __init__(self, inplans, conv_kernels=[3, 5, 7, 9], stride=1, conv_groups=[1, 4, 8, 16]):
        ...
        self.se = SEWeightModule(inplans // 4)
